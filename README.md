# S1-MCA

## DS
1. [Array Operations](DS/array_operations.c)
2. [Bubble Sorting](DS/array_sorting.c)
3. [Array Merge](DS/Array_merge.c)
4. [Stack](DS/stack.c)
5. [Queue](DS/queue.c)
6. [Circular Queue](DS/c_queue.c)
7. [Singly Linked List](DS/linked_list.c)
8. [Doubly Linked List](DS/doubly_ll.c)
9. [Circular Linked List](DS/circular_ll.c)
10. [Set-Bitstring](DS/set.c)
11. [BST](DS/bst.c)
12. [Prims](DS/prims.c)
13. [Kruskal](DS/kruskal.c)
14. [Dijkstras](DS/dijkstras.c)
15. [Topologicaal Sorting](DS/topological_sorting.c)
16. [BFS](DS/bfs.c)
17. [DFS](DS/dfs.c)
18. [Stack using linked list](DS/stack_linkedlist.c)
19. [Queue using linked list](DS/queue_linked_list.c)

## Python
[Python Lab](Python/PythonLab.ipynb)

## Web
[College Portal](Web%20Programming/clgportal/portal.html)

### Javascript
1. [String and Numeric Functions](Web%20Programming/predifinedfns.html)
2. [Change Background Colour](Web%20Programming/backgroundcolor.html)
3. [Change image on hover](Web%20Programming/changeimage.html)
4. [Form Validation](Web%20Programming/formvalidation.html)
5. [Calendar](Web%20Programming/calendar.html)
